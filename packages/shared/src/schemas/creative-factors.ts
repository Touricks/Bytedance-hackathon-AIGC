...
const requiredGuidanceTextSchema = z.string().trim().min(1);
const optionalGuidanceTextSchema = z.string().trim();
...
export const factorGuidanceSchema = z.object({
  ...
  audience: z.object({
    addressingAndTone: optionalGuidanceTextSchema,
    benefitFrame: optionalGuidanceTextSchema,
    sensitivityBoundaries: optionalGuidanceTextSchema,
  }),
  strategy: z.object({
    openingHook: requiredGuidanceTextSchema,
    storyStructure: requiredGuidanceTextSchema,
    evidenceAndCta: requiredGuidanceTextSchema,
  }),
});
...
const AUDIENCE_GUIDANCE = {
  general: {
    factorGuidance: {
      addressingAndTone: "面向广泛用户说服,不强调年龄、身份或特定人群标签。",
      benefitFrame: "聚焦商品/服务本身的通用价值、使用场景和可验证体验。",
      sensitivityBoundaries: "避免强行绑定年龄、性别、身份、家庭角色或焦虑叙事。",
    },
    scriptInfluence: {
      addressee: "广泛用户",
      tone: "清晰可信,聚焦商品/服务本身,不按特定人群定向。",
      benefitPriority: ["通用价值", "使用场景", "可验证体验", "真实感受"],
    },
  },
  toddler: {
    factorGuidance: {
...
const VISUAL_STYLE_SHOT_GUIDANCE = {
  authentic: {
    shotImage:
      "视觉风格：UGC真实质感，自然光或窗边漫射光，曝光充足偏高，色彩干净通透，非广告棚感。",
    shotVideo:
      "运镜质感：镜头固定或轻微手持晃动，动作有物理重量感，无过度运镜，真实UGC节奏。",
    storyboard:
      "场景描写强调真实物理感，商品在自然生活场景中，人物极简（手部/局部优先）。",
  },
  cinematic: {
    shotImage:
      "视觉风格：电影级产品摄影，戏剧性方向光，高饱和高对比，商品是绝对主角，允许完美布光与超现实色彩处理。",
    shotVideo:
      "运镜质感：完美平滑运镜无手持感，允许AI原生视觉效果——极致慢动作、液体流动特效、光线折射、粒子爆散，追求品牌向往感而非真实感。",
    storyboard:
      "场景可以写AI原生视觉：极致慢动作/光线爆发/液体流动特效/超现实材质特写，追求电影级品牌质感，不需要UGC真实感。",
  },
} satisfies Record<
  VisualStyle,
  { shotImage: string; shotVideo: string; storyboard: string }
>;
...
export function buildCreativeFactorRequirements(
  ...
  return creativeFactorRequirementsDataSchema.parse({
    ...compileCreativeRequirementFields({
      factorGuidance,
      scriptInfluence,
      visualStyle: creativeFactors.visualStyle,
    }),
    creativeFactors,
    factorGuidance,
    scriptInfluence,
