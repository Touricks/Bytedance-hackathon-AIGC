...
| `openapi.yaml` | 0530 目标后端 OpenAPI 契约。只保留目标路径，标注 storage binding 与 workspace-scoped 路径。 |
...
5. workspace 与 storage binding 分离：一个 workspace 只能绑定一个 localPath 或 S3 `bucket+prefix`，同一个 storage target 也只能属于一个 workspace；未绑定 storage 时 `/api/workspaces/status` 必须返回 `BIND_STORAGE` 并阻断后续阶段。
