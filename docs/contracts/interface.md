...
Depends On: `docs/contracts/openapi.yaml`, `docs/architecture/runtime-flow.md`, `docs/data/persistence-boundary.md`
...
The server registers routes in `apps/server/src/app.ts` and module controllers under `apps/server/src/modules/*`. `pnpm contract:frontend-backend` verifies that `docs/contracts/openapi.yaml` covers the frontend API surface used by `apps/web`.
...
- Keep `docs/contracts/openapi.yaml` aligned with `apps/server/src/modules/*/*.controller.ts`.
