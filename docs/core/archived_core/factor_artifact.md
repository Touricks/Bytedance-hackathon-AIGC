...
- `creativeFactors` 是主分类标签，包含三个看板主聚合维度和一个默认化的 `visualStyle` 视觉风格控制。
- `factorGuidance` 是主聚合因子展开后的 9 个可编辑细分字段。
- `scriptInfluence` 是主聚合因子对商品卖点、分镜和 shotprompt 的结构化剧本影响缓存。
...
`creativeFactors.productType/audience/strategy` 是数据看板的主聚合标签。`creativeFactors.visualStyle` 参与创作要求编译，但 P0 看板不把它作为主聚合维度。
...
| 值        | 业务含义           | 主要影响                                             |
| --------- | ------------------ | ---------------------------------------------------- |
| `general` | 不限定特定人群     | 不指定年龄、身份或家庭角色，强调普适利益和真实场景。 |
...
| `visual-story`      | 视觉叙事 | 用材质、细节、氛围和视觉连续性承担主要说服结构。                  |

### 2.4 视觉风格

| 值          | 业务含义 | 主要影响 |
| ----------- | -------- | -------- |
| `authentic` | 真实可信 | 写入 `image.style`、`storyboard.rhythm`、`shotImage.global` 和 `shotVideo.global`，强调自然光感、真实素材感和不过度包装。 |
| `cinematic` | 电影感   | 写入同一组编译字段，强调更明确的光影、镜头节奏和商业影像质感。 |
...
`visualStyle` 额外汇入 `image.style`、`storyboard.rhythm`、`shotImage.global` 和 `shotVideo.global`。当前 `compiledRequirementSourceMap` 仍主要解释 9 个可编辑细分字段的来源，不把 `visualStyle` 展开成第 10 个细分字段。
...
| `product-brief`   | 主因子组合、商品角色、证明对象、受众称谓、利益优先级、敏感边界 | 生成商品/服务定义、目标人群、核心卖点、证明对象和禁用表达。                        |
...
- `creativeFactorsRecommendation.recommendedFactors`：包含主因子；`visualStyle` 可省略并由 shared schema 默认成 `authentic`。
...
用户可以在首页修改主因子和视觉风格，也可以展开修改 9 个细分字段。保存时前端提交完整 `prompt_requirements_artifacts.data`：
...
如果用户基于模板修改细分字段，`creativeRequirementTemplate.status="customized"`；如果套用模板后更换了任一主因子或视觉风格，`status="detached"`。模板来源是 secondary tag，不改变主分类。
