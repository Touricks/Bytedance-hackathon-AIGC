...
Workspace LLM modules use file-based subject and contract prompts. Per-shot image and video generation uses deterministic server assembly, then provider workers call Ark Seedream image or Seedance video.
...
| `product-brief` | subject/contract prompt plus material image/runtime context | 商品卖点. |
| `storyboard` | subject/contract prompt plus brief/material context | 分镜脚本. |
