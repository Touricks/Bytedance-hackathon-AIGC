...
完整 assembled prompt 写入 `trace_events.metadata.finalPrompt`，用于调试和真实 provider 追溯。LOCAL workspace 可 append 镜像到 `.daireel/trace/events.jsonl`；S3 workspace 不 append 同一个 JSONL object，而是写 immutable per-event objects：`trace/events/{timestamp}-{uuid}.jsonl` 与 `trace/provider-calls/{timestamp}-{uuid}.jsonl`，避免对象存储并发覆盖。真实 image/video provider 调用写 `trace_events(trace_type='provider_call')`；LOCAL workspace 额外镜像 `.daireel/trace/provider_call.jsonl`，S3 provider-call object 只保存脱敏摘要。事件 schema 为 `provider_call.v1`，记录 job/batch/candidate/attempt、provider/model、media type、latency、generated count、错误、首尾帧或参考图数量、图片参考图来源分类、`promptHash` 和 URL 摘要（host/hash，不保存 signed URL 或 data URL 原文）。图片 provider transport 错误会保留脱敏 endpoint 与底层 cause 摘要；视频 provider 成功行在 Seedance 返回临时 mp4 URL 时写入，`stableUrl=null`，后续 stable 保存耗时由 `asset_persist_started/completed/failed` trace event 表达。写入失败只记录 warn，不阻塞候选生成。
