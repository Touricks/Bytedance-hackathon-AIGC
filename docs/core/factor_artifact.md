...
- `draft`：模型建议的 7 项创作要求草稿。
- `creativeFactorsRecommendation.recommendedFactors`：只包含三个主因子。
- `artifact`：由 `draft + recommendedFactors` 编译得到的 proposed `prompt-requirements` artifact。
...
数据看板不直接按 workspace current requirements 聚合，因为 current 会随用户继续编辑而变化。看板应按**发布记录**聚合，并消费发布记录上的成片标签快照。
