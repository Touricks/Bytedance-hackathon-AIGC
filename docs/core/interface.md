...
| GET | `/api/config/limits` | 返回图像/视频候选批量大小上限和宽高比枚举。 | `{ data }` |
...
| 方法 | 路径 | 业务逻辑 | 请求 |
|---|---|---|---|
| POST | `/api/workspaces` | 新建受管工作区。 | `{ name? }` |
| GET | `/api/workspaces` | 列出工作区及其 active storage binding。 | query 可选 |
