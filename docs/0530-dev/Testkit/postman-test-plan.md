...
|---|---|---|
| `bytedancehack-0530.local-smoke.postman_collection.json` | 快速确认 API path、schema、状态码、DB/storage、shot seed、Campaign/KOL；不触发 text/image/video provider | 不依赖模型 API |
| `bytedancehack-0530.provider.postman_collection.json` | 小批量真实模型联调，覆盖 text/image/video provider、batch polling、select、final video | 依赖 `.env` 中 `TEXT_*`、`IMAGE_*`、`VIDEO_*` |
