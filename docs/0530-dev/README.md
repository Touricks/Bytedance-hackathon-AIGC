...
| `openapi.yaml` | 0530 目标后端 OpenAPI 契约。保留当前兼容路径，并标注建议补齐的 workspace-scoped 路径。 |
...
5. 后端测试先证明契约和状态机正确，再接真实 provider smoke；新版本测试前先执行 `pnpm db:clear -- --yes` 清理 Postgres business tables。
