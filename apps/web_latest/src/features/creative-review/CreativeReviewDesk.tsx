...
function ShotPromptReviewForm({
  ...
  return (
    <section className="review-panel">
      ...
      {editingSummary ? (
        <div className="review-business-form" aria-label="全片分镜生成要求表单">
          <label>
            画幅
            <select
        ...
        {draft.shots.map((shot, index) => {
          ...
          return (
            <article key={`${shot.index}:${index}`} className="shotprompt-card">
              ...
              <Collapse in={isOpen} timeout="auto" unmountOnExit>
                <div id={panelId} className="shotprompt-card__body">
                  {shot.referenceAssetRefs.length > 0 ? (
                    <div className="shotprompt-assets">
                      <span>参考素材</span>
                  ...
                  />
                  <Collapse in={isEditingImage && Boolean(formShot)} timeout="auto" unmountOnExit>
                    {formShot ? (
                  ...
                  <Collapse in={isEditingVideo && Boolean(formShot)} timeout="auto" unmountOnExit>
                    {formShot ? (
                      <ShotPromptLayerEditor
...
function ApplyShotSetPanel({
  ...
  const archivedShotSets = vm.shotSets.filter(
    (shotSet) => shotSet.status === "archived",
  );
  const [historyOpen, setHistoryOpen] = useState(false);
  const [selectedArchivedShotSetId, setSelectedArchivedShotSetId] = useState<
    string | null
  >(null);
  const selectedArchivedShotSet =
    archivedShotSets.find((shotSet) => shotSet.id === selectedArchivedShotSetId) ??
    archivedShotSets[0] ??
    null;
  const selectedArchivedId = historyOpen ? selectedArchivedShotSet?.id : null;
  const archivedShots = useQuery({
    queryKey: ["shot-set-shots", vm.workspaceId, selectedArchivedId],
    queryFn: () => listWorkspaceShotSetShots(vm.workspaceId, selectedArchivedId!),
    enabled: Boolean(selectedArchivedId),
  });
  const archivedShotRows = [...(archivedShots.data?.data ?? [])].sort(
    (a, b) => a.orderIndex - b.orderIndex,
  );
  ...
  const toggleHistory = () => {
    ...
    if (nextOpen && !selectedArchivedShotSetId && archivedShotSets[0]) {
      setSelectedArchivedShotSetId(archivedShotSets[0].id);
    }
  };
  ...
  return (
    <section className="review-panel">
      ...
      <section className="shot-set-history" aria-label="归档分镜链路实例">
        <button
          type="button"
          className="review-secondary shot-set-history__toggle"
          onClick={toggleHistory}
          disabled={archivedShotSets.length === 0}
          aria-expanded={historyOpen}
        >
          {historyOpen ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
          查看归档实例（{archivedShotSets.length}）
        </button>
        {historyOpen ? (
          archivedShotSets.length === 0 ? (
            <p className="review-muted">暂无归档分镜链路实例。</p>
          ) : (
            <div className="shot-set-history__body">
              <div className="shot-set-history__list">
                {archivedShotSets.map((shotSet) => (
                  <button
                    key={shotSet.id}
                    type="button"
                    className={
                      shotSet.id === selectedArchivedShotSet?.id
                        ? "is-selected"
                        : undefined
                    }
                    onClick={() => setSelectedArchivedShotSetId(shotSet.id)}
                    aria-pressed={shotSet.id === selectedArchivedShotSet?.id}
                  >
                    <span className="mono">{shortEntityId(shotSet.id)}</span>
                    <strong>{shotSet.shotCount ?? 0} 个分镜脚本</strong>
                    <small>
                      创建于 {formatReviewTime(shotSet.createdAt)} · 归档于{" "}
                      {formatReviewTime(shotSet.archivedAt)}
                    </small>
                  </button>
                ))}
              </div>
              <div className="shot-set-history__preview">
                {archivedShots.isLoading ? (
                  <p className="review-muted">正在读取归档分镜脚本。</p>
                ) : archivedShotRows.length > 0 ? (
                  <div className="shot-set-shot-grid">
                    {archivedShotRows.map((shot) => (
                      <article key={shot.id} className="shot-set-shot-card">
                        <span className="mono">
                          #{shot.orderIndex} · {formatShotDuration(shot)}
                        </span>
                  ...
                  <p className="review-muted">该归档实例没有可展示的分镜脚本。</p>
                )}
              </div>
            </div>
          )
        ...
        <>
          ...
          {rounds.length > 0 ? (
            <>
              ...
              {isEditingPrompt && latestRound ? (
                <ImagePromptRegenerationForm
                  key={latestRound.artifact.id}
                  artifact={latestRound.artifact}
                  selectedShot={vm.selectedShot}
                  busy={vm.busy || Boolean(vm.hasActiveGeneration)}
                  onSubmit={(userDirection) =>
                    vm.actions.regenerateImage({
                      baseArtifactId: latestRound.artifact.id,
                      userDirection,
                    })
                  }
                />
...
function ImagePromptRegenerationForm({
  ...
  selectedShot: WorkbenchViewModel["selectedShot"];
  busy: boolean;
  onSubmit: (userDirection?: string) => void;
}) {
  ...
  const shotImage = selectedShot?.requirements?.shotImage;
  const shotVideo = selectedShot?.requirements?.shotVideo;
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        event.preventDefault();
        if (busy) return;
        onSubmit(userDirection.trim() || undefined);
      }}
    >
      <div className="review-round__head">
        <span className="review-status">补充生成反馈</span>
        <span>基于第 {artifact.version} 轮和上游分镜生成要求重新生成候选图。</span>
      </div>
      <div className="review-requirement-summary">
        <RequirementSummarySection
          title="分镜图要求"
          value={shotImage}
          labels={SHOT_IMAGE_LABELS}
          order={SHOT_IMAGE_ORDER}
        />
        <RequirementSummarySection
          title="分镜视频要求"
          value={shotVideo}
          labels={SHOT_VIDEO_LABELS}
          order={SHOT_VIDEO_ORDER}
        />
      </div>
      <div className="review-form-grid">
        <label className="review-form-grid__wide">
          ...
          <textarea
            ...
            placeholder="例如：入口标识更清晰，背景更接近参考图，减少画面杂物"
          />
        </label>
      </div>
      <div className="review-panel__actions">
