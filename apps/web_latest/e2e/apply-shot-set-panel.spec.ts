...
async function mockApplyPanelApi(page: Page) {
  ...
  const handleApiRoute = async (route: Route) => {
      ...
      return json(route, {
        data: [
          ...
          },
          {
            id: "ss-archived",
            workspaceId,
            shotPromptArtifactId: "shotprompt-older",
            status: "archived",
            sourceFingerprint: { shotPromptArtifactId: "shotprompt-older" },
            upstream: {
              upstreamChanged: true,
              changedSources: ["shotPromptArtifactId"],
            },
            ...
            archivedAt: "2026-06-02T08:39:00.000Z",
          },
        ],
      });
    }
    ...
    if (
      request.method() === "GET" &&
      path === `/api/workspaces/${workspaceId}/shot-sets/ss-archived/shots`
    ) {
      return json(route, { data: archivedShots });
    }
...
test("apply shot set panel shows active instance, upstream drift, and archived shots", async ({
  page,
}) => {
  ...
  await page.getByRole("button", { name: /查看归档实例（1）/ }).click();

  await expect(page.getByRole("button", { name: /ss-arch/ })).toBeVisible();
  await expect(page.getByText("旧开场")).toBeVisible();
});
