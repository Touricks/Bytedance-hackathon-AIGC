...
function toStoryboardShotRow(row: Record<string, unknown>): StoryboardShotRow {
  return {
    ...
    defaultDurationSec:
      row.default_duration_sec != null
        ? Number(row.default_duration_sec)
        : null,
    status: String(row.status),
    nextAction:
      typeof row.next_action === "string" ? row.next_action : null,
...
function toImagePromptArtifactRow(
  row: Record<string, unknown>,
): ImagePromptArtifactRow {
  return {
    id: String(row.id),
...
function toVideoScriptArtifactRow(
  row: Record<string, unknown>,
): VideoScriptArtifactRow {
  return {
    id: String(row.id),
    shotId: String(row.shot_id),
    version: Number(row.version),
...
function toGenerationJobRow(row: Record<string, unknown>): GenerationJobRow {
  return {
    ...
    queueJobId:
      typeof row.queue_job_id === "string" ? row.queue_job_id : null,
    relatedBatchType:
      typeof row.related_batch_type === "string"
        ? row.related_batch_type
        : null,
    relatedBatchId:
...
function toFinalVideoJobRow(row: Record<string, unknown>): FinalVideoJobRow {
  return {
    ...
    sourceShotVideoIds: Array.isArray(row.source_shot_video_ids)
      ...
      : [],
    sourceVideoScriptArtifactIds: Array.isArray(
      row.source_video_script_artifact_ids,
    )
    ...
    compiledManifestHash:
      typeof row.compiled_manifest_hash === "string"
        ? row.compiled_manifest_hash
        : null,
...
function toTraceEventRow(row: Record<string, unknown>): TraceEventRow {
  return {
    ...
    inputPreview:
      typeof row.input_preview === "string" ? row.input_preview : null,
    outputPreview:
      typeof row.output_preview === "string" ? row.output_preview : null,
...
class PostgresDb2Adapter implements Db2Adapter {
  ...
  async listImagePromptArtifacts(
    shotId: string,
  ): Promise<ImagePromptArtifactRow[]> {
    const result = await this._pool.query(
  ...
  async listImageCandidatesByBatch(
    batchId: string,
  ): Promise<ImageCandidateRow[]> {
    const result = await this._pool.query(
  ...
  async upsertSelectedImage(input: {
    ...
    await this._pool.query(
      `insert into selected_images (shot_id, image_candidate_id, image_generation_batch_id)
       values ($1, $2, $3)
      ...
      [input.shotId, input.imageCandidateId, input.imageGenerationBatchId],
    );
  }
  ...
  async getSelectedImage(
    ...
    const result = await this._pool.query(
      "select * from selected_images where shot_id = $1",
      [shotId],
    );
    const row = result.rows[0];
    if (!row) return null;
  ...
  async listVideoScriptArtifacts(
    shotId: string,
  ): Promise<VideoScriptArtifactRow[]> {
    const result = await this._pool.query(
  ...
  async listVideoCandidatesByBatch(
    batchId: string,
  ): Promise<VideoCandidateRow[]> {
    const result = await this._pool.query(
  ...
  async upsertSelectedVideo(input: {
    ...
    await this._pool.query(
      `insert into selected_videos (shot_id, video_candidate_id, video_generation_batch_id)
       values ($1, $2, $3)
       on conflict (shot_id) do update
       set video_candidate_id = excluded.video_candidate_id,
           video_generation_batch_id = excluded.video_generation_batch_id`,
      [input.shotId, input.videoCandidateId, input.videoGenerationBatchId],
    );
  }

  async deleteSelectedVideo(shotId: string): Promise<void> {
    await this._pool.query(
      "delete from selected_videos where shot_id = $1",
      [shotId],
    );
  }
  ...
  async getFinalVideoJobByIdempotencyKey(
    key: string,
  ): Promise<FinalVideoJobRow | null> {
    const result = await this._pool.query(
