...
The V2 Postman agent-chain assets remain under `test/postman/agent-chain/` for contract reference, but they are not an active automated real-provider test entry.

Legacy V1 Postman/OpenAPI files were removed from the active Postman test assets because the V1 static workspace builder endpoints are intentionally no longer registered.
